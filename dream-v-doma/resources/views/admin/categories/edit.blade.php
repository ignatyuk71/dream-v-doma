...
<div class="container py-4">
    ...
    <form method="POST" action="{{ route('admin.categories.update', $category->id) }}">
        ...
        <ul class="nav nav-tabs mb-3" id="langTabs" role="tablist">
            <li class="nav-item">
                <button type="button" class="nav-link active" data-bs-toggle="tab" data-bs-target="#uk">Українська</button>
            </li>
            <li class="nav-item">
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane fade {{ $locale == 'uk' ? 'show active' : '' }}" id="{{ $locale }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="translations[{{ $locale }}][name]" class="form-control"
                        value="{{ $category->translations->firstWhere('locale', $locale)?->name }}" required>
                </div>
                <div class="mb-3">
                    <label class="form-label">Meta Title ({{ $locale }})</label>
                    <input type="text" name="translations[{{ $locale }}][meta_title]" class="form-control"
                        value="{{ $category->translations->firstWhere('locale', $locale)?->meta_title }}" required>
                </div>
                <div class="mb-3">
                    <label class="form-label">Meta Description ({{ $locale }})</label>
                    <textarea name="translations[{{ $locale }}][meta_description]" class="form-control" required>{{ $category->translations->firstWhere('locale', $locale)?->meta_description }}</textarea>
                </div>
            </div>
            @endforeach
        </div>
            ...
            <select name="parent_id" class="form-select">
                ...
                @foreach ($parentCategories as $parent)
                    <option value="{{ $parent->id }}" {{ $category->parent_id == $parent->id ? 'selected' : '' }}>
                        {{ $parent->translations->firstWhere('locale', 'uk')?->name ?? '—' }}
                    </option>
                @endforeach
            </select>
        </div>
