...
const attributes = reactive({
  uk: props.modelValue.uk ? JSON.parse(JSON.stringify(props.modelValue.uk)) : [],
  ru: props.modelValue.ru ? JSON.parse(JSON.stringify(props.modelValue.ru)) : [],
})
...
// Слідкуємо за локальними змінами та повертаємо наверх
watch(
  attributes,
  (v) => {
    emits('update:modelValue', {
      uk: v.uk.map(({_id, ...rest}) => rest.name || rest.value ? {...rest} : null).filter(Boolean),
      ru: v.ru.map(({_id, ...rest}) => rest.name || rest.value ? {...rest} : null).filter(Boolean)
    })
  },
  { deep: true }
)
